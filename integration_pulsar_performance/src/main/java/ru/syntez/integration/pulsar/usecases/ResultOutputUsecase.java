...
public class ResultOutputUsecase {
    ...
    public static void execute(List<ResultReport> promiseList, int producerCount, int timeOutSeconds) {
        ...
        Double publishLatency = promiseList.stream().filter(value -> !value.getIsProducer()).mapToDouble(value -> value.getPublishLatency().doubleValue()).max().getAsDouble();
        Double endToEndLatency = promiseList.stream().filter(value -> !value.getIsProducer()).mapToDouble(value -> value.getEndToEndLatency().doubleValue()).max().getAsDouble();
        ...
        LOG.info(String.format("Publish latency мл: %s", publishLatency));
        LOG.info(String.format("EndToEnd latency мл: %s", endToEndLatency));
    }
